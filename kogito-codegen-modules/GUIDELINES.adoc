...
- `<Runtime>` is to have platform specific templates: `Quarkus`/`Java`
...
2. Specialize runtime-specific annotation/behavior (Quarkus) using templates if possible (inherit or call library methods)
...
We provide an Annotator for Quarkus (CDI). Annotators decorate methods of a template using framework-specific annotations.
...
for a field, depending whether we are generating code for CDI.
