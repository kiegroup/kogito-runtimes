...
public class AddonsConfigDiscovery {
    ...
    public static AddonsConfig discover(Predicate<String> classAvailabilityResolver, Predicate<Class<?>> classSubTypeAvailabilityResolver) {
        ...
        boolean useCloudEvents = classAvailabilityResolver.test(QUARKUS_CLOUD_EVENTS);
        boolean useExplainability = classAvailabilityResolver.test(QUARKUS_EXPLAINABILITY);
        boolean useProcessSVG = classAvailabilityResolver.test(QUARKUS_PROCESS_SVG);
        ...
        boolean useSourceFiles = classAvailabilityResolver.test(QUARKUS_SOURCE_FILES_CLASS);
